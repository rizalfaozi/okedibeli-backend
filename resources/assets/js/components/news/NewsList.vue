<template>
    <div>
        ...
        <div class="pull-left mb10 mtn10">
            ...
            <strong class="ml5">Total News : {{ lists.total }}</strong>
        </div>
       ...
       <button class="btn btn-default pull-right mb10 mtn10" @click="refreshCache"><i class="fa fa-refresh"></i> Refresh</button>
        ...
        <ViewBox v-if="showModalView" @close="showModalView=false" :detail="viewProfile"></ViewBox>
...
<script>
    ...
    export default {
        ...
        components: {
            'ListIndex': require('./NewsListIndex.vue').default,
            'SearchBox': require('./NewsListSearch.vue').default,
            'Pagination': require('vue-plain-pagination'),
            'ViewBox': require('./NewsViewInfo.vue').default,
        ...
        methods: {
            ...
            buildSearchMessage(params) {
                ...
                if(params.summary != "") {
                    params_data += "summary = " + params.summary;
                }
                if(params.description != "") {
                    params_data += "description = " + params.description;
                }
            ...
            getListData() {
                ...
                if (this.isSearch) {
                    q = {
                        ...
                        summary: this.searchParams.summary,
                        description: this.searchParams.description,
                        status: this.searchParams.status,

                        page: self.halaman
                    }
                    listUrl = BASE_URL + '/api/news/search';
                }else{
                    q = {page: self.halaman}
                    listUrl = BASE_URL + '/api/news/lists';    
                }
