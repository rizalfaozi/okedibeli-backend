            ...
            <div class="panel-body">
                <form role="form" enctype="multipart/form-data" method="post" class="form-horizontal" @submit.prevent="postData">
                    <div class="form-group" :class="results.messages.name ? 'has-error' : ''">
                        ...
                        <div class="col-md-8">
                            <input type="text" v-model="name" class="form-control" placeholder="Title">
                            <label v-if="results.messages.name !== ''" for class="control-label">{{ results.messages.name }}</label>
                        </div>
                    </div>

                    <div class="form-group" :class="results.messages.summary ? 'has-error' : ''">
                        <label class="col-md-2 control-label">Summary</label>
                        <div class="col-md-8">
                            <input type="text" v-model="summary" class="form-control" placeholder="Summary">
                            <label v-if="results.messages.summary !== ''" for class="control-label">{{ results.messages.summary }}</label>
                        </div>
                    </div>

                    <div class="form-group" :class="results.messages.url_demo ? 'has-error' : ''">
                        <label class="col-md-2 control-label">URL Demo</label>
                        <div class="col-md-8">
                            <input type="text" v-model="url_demo" class="form-control" placeholder="URL Demo">
                            <label v-if="results.messages.url_demo !== ''" for class="control-label">{{ results.messages.url_demo }}</label>
                        </div>
                    </div>
                    ...
                    <div class="form-group" :class="results.messages.price ? 'has-error' : ''">
                        <label class="col-md-2 control-label">Price</label>
                        <div class="col-md-8">
                            <input type="text" v-model="price" class="form-control" placeholder="Price">
                            <label v-if="results.messages.price !== ''" for class="control-label">{{ results.messages.price }}</label>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            editData(){
                ...
                if(this.$route.params.id){
                    console.log("masuk");
                    axios.get(BASE_URL+'/api/themes/edit/'+this.userID)
                    .then((response) => {
                        if(response.data.id){
                            this.edit = true;
                            this.loading = false;
                            ...
                            this.url_demo=response.data.url_demo;
            ...
            postData() {
                ...
                formData.append('url_demo', this.url_demo);
                ...
                if(this.edit){
                    ...
                    urlBase = axios.post(BASE_URL+'/api/themes/update/'+this.userID, formData, config);
                }else{
                    urlBase = axios.post(BASE_URL+'/api/themes/post', formData, config);
                }
