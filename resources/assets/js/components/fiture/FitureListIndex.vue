        ...
        methods: {
            someDelete(data){
                ...
                this.$swal({
                    ...
                    if (result) {
                        ...
                        axios.get(BASE_URL + '/api/fiture/delete/'+this.data.id)
                        .then((response) => {
            ...
            btnActive() {
                ...
                if(self.data.status == 1){
                    self.urlID = self.data.id+'/0';
                }else{
                    self.urlID = self.data.id+'/1';
                }
                self.isCircle = false;
                self.isSpinner = true;
                axios.post(BASE_URL + '/api/fiture/status/'+self.urlID).then((response) => {
