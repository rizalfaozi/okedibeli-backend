                ...
                <form role="form" enctype="multipart/form-data" method="post" class="form-horizontal" @submit.prevent="postData">
                    <div class="form-group" :class="results.messages.title ? 'has-error' : ''">
                        ...
                        <div class="col-md-8">
                            ...
                            <label v-if="results.messages.judul !== ''" for class="control-label">{{ results.messages.title }}</label>
                        </div>
                    </div>
    ...
    export default {
        ...
        methods: {
            ...
            editData(){
                ...
                if(this.$route.params.id){
                    console.log("masuk");
                    axios.get(BASE_URL+'/api/panduan/edit/'+this.userID)
                    .then((response) => {
                        if(response.data.id){
                            this.edit = true;
                            this.loading = false;
            ...
            postData() {
                ...
                if(this.edit){
                    ...
                    urlBase = axios.post(BASE_URL+'/api/fiture/update/'+this.userID, formData, config);
                }else{
                    urlBase = axios.post(BASE_URL+'/api/fiture/store', formData, config);
                }
