<template>
    <div ref="listIndex">
        ...
        <SearchBox v-if="showModalSearch" @close="showModalSearch=false" @search="searchData"></SearchBox>
         <ViewBox v-if="showModalView" @close="showModalView=false" :dataMess="viewProfile"></ViewBox>
    </div>
</template>
<script>
    ...
    export default {
        ...
        components: {
            'ListIndex': require('./MessagesListIndex.vue').default,
            'SearchBox': require('./MessagesListSearch.vue').default,
            'Pagination': require('vue-plain-pagination'),
            'ViewBox': require('./MessagesViewInfo.vue').default,
        },
        methods: {
            prosesloading(val){
                this.loading = val;
            ...
            getListData() {
                ...
                if (this.isSearch) {
                    ...
                    listUrl = BASE_URL + '/api/messages/search';
                }else{
                    q = {page: self.halaman}
                    listUrl = BASE_URL + '/api/messages/lists';    
                }
